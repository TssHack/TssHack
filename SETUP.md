...
2. Copy **the contents** of `tsshack` into it, excluding `.git` and `.venv` if they exist locally.
...
The current generator contains the `A`, `R`, and `T` glyphs needed for `ehsan`. Add another glyph to `GLYPHS` before using a different letter.
...
- Replace `ehsan@github` in the terminal headings.
...
```text
Set up the tsshack Terminal GitHub profile template in this repository for me.
